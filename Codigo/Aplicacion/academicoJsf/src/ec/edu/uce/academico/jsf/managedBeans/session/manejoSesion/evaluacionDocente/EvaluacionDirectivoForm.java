 ...
 * 
 * @author ajvilledfuerte.
 * @version 1.0
 */

@ManagedBean(name = "evaluacionDirectivoForm")
@SessionScoped
public class EvaluacionDirectivoForm implements Serializable {
	...
	@EJB
	FuncionTipoEvaluacionServicio servEdfFuncionTipoEvaluacionServicio;
	@EJB
	TipoContenidoServicio servEdfTipoContenidoServicio;
	@EJB
	ContenidoServicio servEdfContenidoServicio;
	@EJB
	EvaluadorEvaluadoServicio servEdfEvaluadorEvaluadoServicio; 
	@EJB
	AsignacionEvaluadorServicio servEdfAsignacionEvaluadorServicio;
	@EJB
	TpcnFuncionTpevServicio servEdfTpcnFuncionTpevServicio;
	@EJB
	ContenidoEvaluacionDtoServicioJdbc servEdfContenidoEvaluacionDtoJdbcServicio;
	...
	public String irEvaluacionDirectivoDocente(Usuario usuario) {
		...
		return "irEvaluacionDirectivoDocente";
	}
	...
	public Boolean validarCronograma(){
		...
		if(edfEvaluacionActiva != null){
			if(edfEvaluacionActiva.getEvaCronogramaFin().after(fechaActual) || edfEvaluacionActiva.getEvaCronogramaFin().equals(fechaActual) ){
				if(edfEvaluacionActiva.getEvaCronogramaInicio().before(fechaActual) || edfEvaluacionActiva.getEvaCronogramaInicio().equals(fechaActual)){ 
					verificar = true;
				}else{
					FacesUtil.mensajeError(MensajeGeneradorUtilidades.getMsj(new MensajeGeneradorUtilidades("EvaluacionDirectivo.validar.cronograma.no.iniciado.validacion.exception")));
				}
			}else{
				FacesUtil.mensajeError(MensajeGeneradorUtilidades.getMsj(new MensajeGeneradorUtilidades("EvaluacionDirectivo.validar.cronograma.expirado.validacion.exception")));
			}
		}
		return verificar;
	}
