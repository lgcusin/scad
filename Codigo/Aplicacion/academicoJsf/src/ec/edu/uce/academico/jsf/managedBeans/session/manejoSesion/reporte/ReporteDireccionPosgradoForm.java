 ...
 ************************************************************************* 
 ...
 ARCHIVO:     ReporteDireccionPosgradoForm.java	  
 DESCRIPCION: Managed Bean que maneja las peticiones del reporte de posgrados.
 *************************************************************************
                           	MODIFICACIONES
                            
 FECHA         		     AUTOR          					COMENTARIOS
29-ENERO-2019			Daniel Albuja                  Emisión Inicial
 ***************************************************************************/
...
/**
 * Clase (managed bean) ReporteDireccionPosgradoForm
 * Managed Bean que maneja las peticiones del reporte de posgrado de DGPP.
 * @author dalbuja.
 * @version 1.0
 */

@ManagedBean(name="reporteDireccionPosgradoForm")
@SessionScoped
public class ReporteDireccionPosgradoForm implements Serializable{
	...
	public String irReportePosgrado(Usuario usuario) { 
		rpfUsuario= usuario;
		try {
			List<UsuarioRol> usro = servRpfUsuarioRolServicio.buscarXUsuario(usuario.getUsrId());
			for (UsuarioRol item : usro) {
				if(item.getUsroRol().getRolId() == RolConstantes.ROL_ADMINDPP_VALUE.intValue()){
			...
			return "irReporteGeneralPosgrado";
...
//	}
	...
	public void iniciarParametrosPosgrado() {
		try {
			...
			rpfValidadorClick = 0;
			rpfValidadorEdicion = 0;
			rpfTipoUsuario= RolConstantes.ROL_ADMINDPP_VALUE;
			llenarPeriodos(); //Listar todos los peridos academicos
		...
		try {
			...
			if(rpfTipoUsuario == RolConstantes.ROL_SECRECARRERA_VALUE.intValue() || rpfTipoUsuario == RolConstantes.ROL_DIRCARRERA_VALUE.intValue() || rpfTipoUsuario == RolConstantes.ROL_SOPORTE_VALUE.intValue()){
				...
				if(rpfTipoUsuario == RolConstantes.ROL_SOPORTE_VALUE.intValue()){
					...
					rpfListCarreraDto = servRpfCarreraDto.buscarTodasPosgrado();
				}
	...
	public void buscarEstudiantesPosgrado(){
		try {
			rpfListaEstudiantes=servEstudianteDtoServicioJdbc.buscarEstudianteXPeriodoXProgramaPosgradoDireccionPosgrado(rpfParaleloDtoEditar.getPracId(), rpfCarreraDto.getCrrId());
			List<EstudianteJdbcDto> listaPosgrado = new ArrayList<EstudianteJdbcDto>();
			for (EstudianteJdbcDto item : rpfListaEstudiantes) {
				item= servEstudianteDtoServicioJdbc.buscarEstudianteMatriculaPosgrado(item,rpfParaleloDtoEditar.getPracId(), rpfCarreraDto.getCrrId());
				listaPosgrado.add(item);
			}
			rpfListaEstudiantes = listaPosgrado;
			generarReporteEstados(listaPosgrado);
	...
	public static void generarReporteEstados(List<EstudianteJdbcDto> listaEstados){
		...
		// ****************************************************************//
		// ****************************************************************//
		frmCrpNombreReporte = "ReporteEstadosPosgradistas";
		...
		for (EstudianteJdbcDto item : listaEstados) {
			...
			dato.put("periodo",item.getPracDescripcion());
			...
			dato.put("programa", item.getCrrDescripcion());
			dato.put("email", item.getPrsMailPersonal());
			dato.put("correo_i", item.getPrsMailInstitucional());
			dato.put("telefono", item.getPrsTelefono());
			if(item.getRcesEstado()!=null){
				if(item.getRcesEstado()==RecordEstudianteConstantes.ESTADO_INSCRITO_VALUE){
					dato.put("matricula", RecordEstudianteConstantes.ESTADO_INSCRITO_LABEL);	
					dato.put("pago", GeneralesConstantes.APP_SI_LABEL);
				}else{
					dato.put("matricula", RecordEstudianteConstantes.ESTADO_MATRICULADO_LABEL);
					dato.put("pago", GeneralesConstantes.APP_NO_LABEL);
				}
			}else{
				dato.put("matricula", "SIN MATRICULAR");
			}
			if(item.getCmpaTotalPago()!=null){
				dato.put("arancel", item.getCmpaTotalPago().toString());
			}else{
				dato.put("arancel", "SIN MATRICULAR");
			}
			if(item.getCmpaNumComprobante()!=null){
				dato.put("comprobante", item.getCmpaNumComprobante().toString());
			}else{
				dato.put("comprobante", "SIN MATRICULAR");
			}
			if(item.getFcmtFechaMatricula()!=null){
				dato.put("fechamatricula", item.getFcmtFechaMatricula().toString());
			}else{
				dato.put("fechamatricula", "SIN MATRICULAR");
			}
