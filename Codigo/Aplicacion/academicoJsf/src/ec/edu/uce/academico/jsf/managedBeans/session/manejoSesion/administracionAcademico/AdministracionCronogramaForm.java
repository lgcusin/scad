 ...
 ************************************************************************* 
 ...
 ARCHIVO:     AdministracionCronogramaForm.java	  
...
/**
 * Clase (managed bean) AdministracionCronogramaForm.
 * Managed Bean que maneja las peticiones para la administración de la tabla Cronograma.
 * @author jvrosales.
 * @version 1.0
 */

@ManagedBean(name="administracionCronogramaForm")
@SessionScoped
public class AdministracionCronogramaForm implements Serializable{
	 ...
	 */
	public String irAdministracionCronograma(Usuario usuario) {
		...
		try {
			...
			for (UsuarioRol item : usro) {
				if(item.getUsroRol().getRolId() == RolConstantes.ROL_ADMINDGA_VALUE.intValue() || item.getUsroRol().getRolId() == RolConstantes.ROL_SOPORTE_VALUE.intValue()){
					acfTipoUsuario = RolConstantes.ROL_ADMINDGA_VALUE.intValue();
					acfTipoCarrera="carreras";
				}else if (item.getUsroRol().getRolId() == RolConstantes.ROL_ADMINDPP_VALUE.intValue()){
					acfTipoUsuario = RolConstantes.ROL_ADMINDPP_VALUE.intValue();
		...
		iniciarParametros();
		return "irAdministracionCronograma";
	}
	...
	public void iniciarParametros(){
		...
		if(acfTipoUsuario == RolConstantes.ROL_ADMINDGA_VALUE.intValue() || acfTipoUsuario == RolConstantes.ROL_SOPORTE_VALUE.intValue()){
				
			try {
				acfListPeriodoAcademico = servAcfPeriodoAcademicoServicio.buscar(new Integer [] {PeriodoAcademicoConstantes.ESTADO_ACTIVO_VALUE, PeriodoAcademicoConstantes.ESTADO_EN_CIERRE_VALUE}, new Integer[] {PeriodoAcademicoConstantes.PRAC_SUFICIENCIA_INFORMATICA_VALUE, PeriodoAcademicoConstantes.PRAC_SUFICIENCIA_CULTURA_FISICA_VALUE, PeriodoAcademicoConstantes.PRAC_IDIOMAS_VALUE, PeriodoAcademicoConstantes.PRAC_PREGRADO_VALUE} );
		...
		}else if (acfTipoUsuario == RolConstantes.ROL_ADMINDPP_VALUE.intValue()){
			try {
				acfListPeriodoAcademico = servAcfPeriodoAcademicoServicio.listarXEstadoPosgrado(PeriodoAcademicoConstantes.ESTADO_ACTIVO_VALUE);
				acfListaCombos = lista.getListaTipoCronogramaPosgrado();
			} catch (PeriodoAcademicoDtoJdbcException e) {
	...
	public void buscar(Integer pracId, Integer crnTipo){
		...
		if(pracId != GeneralesConstantes.APP_ID_BASE){
				...
				try {
					...
					if (periodo != null) {
						if (periodo.getPracTipo().intValue() == PeriodoAcademicoConstantes.PRAC_PREGRADO_VALUE) {
							if (crnTipo.intValue() != GeneralesConstantes.APP_ID_BASE) {
								if (crnTipo.intValue() == CronogramaConstantes.TIPO_NIVELACION_VALUE) {
									acfListCronograma = cargarProcesosCronograma(pracId, CronogramaConstantes.TIPO_NIVELACION_VALUE);
								}
								
								if (crnTipo.intValue() == CronogramaConstantes.TIPO_ACADEMICO_VALUE) {
									acfListCronograma = cargarProcesosCronograma(pracId, CronogramaConstantes.TIPO_ACADEMICO_VALUE);	
								}
								if (crnTipo.intValue() == CronogramaConstantes.TIPO_VERANO_NIVELACION_EN_LINEA_VALUE) {
									acfListCronograma = cargarProcesosCronograma(pracId, CronogramaConstantes.TIPO_VERANO_NIVELACION_EN_LINEA_VALUE);	
								}	
							}else {
								acfListCronograma = null;
								FacesUtil.mensajeInfo("Seleccione un tipo de Cronograma.");
							}
						}else if (periodo.getPracTipo().intValue() == PeriodoAcademicoConstantes.PRAC_IDIOMAS_VALUE) {
							acfCronogramaId = GeneralesConstantes.APP_ID_BASE;
							acfListCronograma = cargarProcesosCronograma(pracId, CronogramaConstantes.TIPO_SUFICIENCIA_IDIOMAS_VALUE);
						}else if (periodo.getPracTipo().intValue() == PeriodoAcademicoConstantes.PRAC_SUFICIENCIA_CULTURA_FISICA_VALUE) {
							acfCronogramaId = GeneralesConstantes.APP_ID_BASE;
							acfListCronograma = cargarProcesosCronograma(pracId, CronogramaConstantes.TIPO_SUFICIENCIA_CULTURA_FISICA_VALUE);
						}else if (periodo.getPracTipo().intValue() == PeriodoAcademicoConstantes.PRAC_SUFICIENCIA_INFORMATICA_VALUE) {
							acfCronogramaId = GeneralesConstantes.APP_ID_BASE;
							acfListCronograma = cargarProcesosCronograma(pracId, CronogramaConstantes.TIPO_SUFICIENCIA_INFORMATICA_VALUE);
						}
					}
