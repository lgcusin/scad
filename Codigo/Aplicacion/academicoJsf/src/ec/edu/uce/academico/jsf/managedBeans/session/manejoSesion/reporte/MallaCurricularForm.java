 ...
 ************************************************************************* 
 ...
 ARCHIVO:     MallaCurricularForm.java	  
 DESCRIPCION:  Managed Bean que maneja las peticiones para los reportes de las Mallas Curriculares.
 *************************************************************************
                           	MODIFICACIONES
                            
 FECHA         		     AUTOR          					COMENTARIOS
23-Enero-2018			 Freddy Guzmán 						Emisión Inicial
 ***************************************************************************/
...
/**
 * Clase (managed bean) MallaCurricularForm.
 * Managed Bean que maneja las peticiones para los reportes de las Mallas Curriculares.
 * @author fgguzman.
 * @version 1.0
 */
@ManagedBean(name="mallaCurricularForm")
@SessionScoped
public class MallaCurricularForm implements Serializable{
	...
	private List<Carrera> amcfListCarrera;
	...
	public String irMallaCurricular(Usuario usuario){
		...
		try {
			amcfUsuario = usuario;
			servUsuarioRol.buscarXUsuario(usuario.getUsrId());
			amcfPeriodoAcademicoDto = cargarPeriodoAcademicoActivo();
			
			try {

				amcfListCarrera = new ArrayList<>();
				List<RolFlujoCarrera> rolflujocarrera = servRolFlujoCarrera.buscarXUsuarioXRoles(amcfUsuario.getUsrId(), new Integer[]{RolConstantes.ROL_SECRECARRERA_VALUE, RolConstantes.ROL_SECRESUFICIENCIAS_VALUE});
				for(RolFlujoCarrera rlflcr: rolflujocarrera){ 
					amcfListCarrera.add(servCarrera.buscarPorId(rlflcr.getRoflcrCarrera().getCrrId()));
				}
		...
		return "irListarMallaCurricularSecretaria";
	}
	...
	public void buscarMallas(){
		...
		try {
			if (!amcfCarreraId.equals(GeneralesConstantes.APP_ID_BASE)) {
				amcfCarrera = servCarrera.buscarPorId(amcfCarreraId);
				amcfListMallaCurricularDto =  servJdbcMallaCurricularDto.buscarMallaCurricular(amcfCarrera.getCrrDependencia().getDpnId(), amcfCarreraId, new Integer[]{CarreraConstantes.TIPO_PREGRADO_VALUE}, new Integer[]{MallaCurricularConstantes.ESTADO_MALLA_ACTIVO_VALUE});
			}else {
	...
	public void imprimirMallaCurricular(MallaCurricularDto malla){
		...
		if (!materias.isEmpty()) {
			amcfTipoArchivo = "PDF";
			if (amcfCarrera.getCrrProceso().equals(CarreraConstantes.PROCESO_PREGRADO_REDISENO_VALUE)) {
	...
	public List<Carrera> getAmcfListCarrera() {
		return amcfListCarrera;
	}

	public void setAmcfListCarrera(List<Carrera> amcfListCarrera) {
		this.amcfListCarrera = amcfListCarrera;
	}

	public Carrera getAmcfCarrera() {
		return amcfCarrera;
	}

	public void setAmcfCarrera(Carrera amcfCarrera) {
		this.amcfCarrera = amcfCarrera;
	}
