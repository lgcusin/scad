...
@ManagedBean(name="reporteDocenciaParaleloForm")
@SessionScoped
public class ReporteDocenciaPorParaleloForm implements Serializable{
	...
	public void buscarDocenciaPorParalelo(){
		try {
			if (rpdfPeriodoAcademico.getPracId() == GeneralesConstantes.APP_ID_BASE) {
				...
				rpdfListDocentesMateriasBusq =servDocenteDtoServicioJdbc.listarDocenciaXParalelo(rpdfPeriodoAcademico.getPracId(),rpdfDependenciaDto.getDpnId(), rpdfCarreraDto.getCrrId(), rpdfUsuario, rpdfTipoUsuario);
	...
	public void cargarReporte(){
		try {
			...
			frmCrpParametros.put("periodo",rpdfListPeriodoAcademico.get(0).getPracDescripcion());
			if(rpdfCarreraDto.getCrrId()==GeneralesConstantes.APP_ID_BASE){
