...
public class Principal {
	...
	public void init() {
		...
		if (beanLogin.isDocente()) {
			fdId = beanLogin.getUsuarioRol().getUsroUsuario().getUsrPersona().getPrsFichaDocentes().get(0).getFcdcId();
			fcId = beanLogin.getUsuarioRol().getUsroUsuario().getUsrPersona().getPrsFichaDocentes().get(0)
					.getFcdcDetallePuestos().get(0).getDtpsCarrera().getCrrDependencia().getDpnId();
		} else if (beanLogin.isAdminFacultad()) {
			fcId = beanLogin.getUsuarioRol().getUsroUsuario().getUsrPersona().getPrsFichaEmpleados().get(0)
					.getFcemDetallePuestos().get(0).getDtpsCarrera().getCrrDependencia().getDpnId();
		}
	}
