...
public class Syllabus {
	...
	public void init() {
		...
		if (beanLogin.isDocente()) {
			...
			lstC = srvSgm.listarAllCrrByFcdc(beanLogin.getUsuarioRol().getUsroUsuario().getUsrPersona()
					.getPrsFichaDocentes().get(0).getFcdcId());
			lstM = srvSgm.listarAllMatByFcdc(beanLogin.getUsuarioRol().getUsroUsuario().getUsrPersona()
					.getPrsFichaDocentes().get(0).getFcdcId());
		}
	...
	public String verDetaSyllabus() {
		...
		if (selectMtr.getMtrId() > 0) {// Verifica lamateria seleccionada.
	...
	public String AddNewUnidadCurricular() {
		...
		if (uc.getUncrDescripcion() == null) {
			...
			// uc.setUncrDescripcion("UNIDAD CURRICULAR No. " +
			// lstUC.indexOf(uc) + 1);
