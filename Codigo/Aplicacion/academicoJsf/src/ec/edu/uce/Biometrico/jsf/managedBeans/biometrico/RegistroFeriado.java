...
import ec.edu.uce.academico.ejb.servicios.biometrico.interfaces.SrvRegistroFeriadoLocal;
...
public class RegistroFeriado {
	...
	private boolean validarCarrera() {
		if (selectCarrera != null && selectCarrera.getCrrId() > 0) {
			return true;
		} else {
	...
	private boolean validarDependencia() {
		if (selectDependencia != null && selectDependencia.getDpnId() > 0) {
			return true;
		} else {
			FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_ERROR,
