...
public class RegistroHorarioExamen {
	...
	public void init() {
		...
		lstC = srvEmp.listarCarreras(
				dataLogin.getUsuarioRol().getUsroUsuario().getUsrPersona().getPrsFichaDocentes().get(0).getFcdcId());
		iniciar();
	}
	...
	public void setCarreraID(ValueChangeEvent event) {
		if (event.getNewValue() != null) {
			...
			selectSem.setNvlId(0);
			selectMtr.setMtrId(0);
		...
		if (lstAsistenciasByHorario.isEmpty()) {
			...
			for (HorarioAcademico h : lstHorarios) {
				...
				for (HorarioAcademico h2 : lstHorarios) {
					if (h.getHracDia().equals(h2.getHracDia()) && h.getHracMallaCurricularParalelo().getMlcrprId() == h2
			...
			for (HorarioAcademico h : lstAux) {
				...
				asis.setFichaDocente(
						dataLogin.getUsuarioRol().getUsroUsuario().getUsrPersona().getPrsFichaDocentes().get(0));
