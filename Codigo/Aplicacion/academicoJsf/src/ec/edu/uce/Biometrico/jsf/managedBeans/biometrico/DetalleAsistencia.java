			...
			try {
				if (validarFechas()) {
					...
					if (beanLogin.Docente) {
						lstA = srvDnt.listarAsistencia(beanLogin.getUsuarioRol().getUsroUsuario().getUsrPersona()
								.getPrsFichaDocentes().get(0).getFcdcId(), inicio, fin, null);
	...
	private ArrayList<String[]> getAsistenciaDocentes() {
		...
		for (Asistencia asistencia : lstA) {
			rows.add(new String[] { getFormatoFecha(asistencia),
					asistencia.getHorarioAcademico().getHracMallaCurricularParalelo().getMlcrprMallaCurricularMateria()
							.getMlcrmtMateria().getMtrDescripcion(),
