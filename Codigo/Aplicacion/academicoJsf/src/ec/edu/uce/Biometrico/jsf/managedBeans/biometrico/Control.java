...
import ec.edu.uce.academico.ejb.servicios.biometrico.interfaces.JSDCLocal;
...
public class Control {
	...
	public void init() {
		...
		fclId = l.getUsuarioRol().getUsroUsuario().getUsrPersona().getPrsFichaEmpleados().get(0).getFcemDetallePuestos().get(0)
	...
	private String verHorarioFueraTiempo() {
		...
		for (Asistencia ass : lstAuxA) {
			if (ass.getHorarioAcademico().getHracHoraClaseAula().getHoclalHoraClase().getHoclHoraInicio() <= ahora
					.getHours()) {
	...
	private String inicializarClase(Asistencia asistencia, String estado) {
		...
		lstCnt = srvSgmt.buscarContenidos(
				hrrI.getHracMallaCurricularParalelo().getMlcrprMallaCurricularMateria().getMlcrmtMateria().getMtrId());
		if (lstCnt.isEmpty()) {
			...
			lstSgmt = srvSgmt.getSeguimiento(hrrI.getHracMallaCurricularParalelo().getMlcrprMallaCurricularMateria()
					.getMlcrmtMateria().getMtrId(), regDcnt.getFcdcId());
			horaClase = srvSgmt.obtenerHoraClasexHorario(hrrI);
			int tiempo = hrrI.getHracHoraClaseAula().getHoclalHoraClase().getHoclHoraFin()
					- hrrI.getHracHoraClaseAula().getHoclalHoraClase().getHoclHoraInicio();
	...
	private String finalizarClase(Asistencia asistencia) {
		lstCnt = srvSgmt.buscarContenidos(
				hrrF.getHracMallaCurricularParalelo().getMlcrprMallaCurricularMateria().getMlcrmtMateria().getMtrId());
		if (lstCnt.isEmpty()) {
			FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_FATAL, "No se encontraron temas de clases",
					"Verifique si existen el syllabus respectivo");
			FacesContext.getCurrentInstance().addMessage(null, msg);
			...
			lstSgmt = srvSgmt.getSeguimiento(hrrF.getHracMallaCurricularParalelo().getMlcrprMallaCurricularMateria()
					.getMlcrmtMateria().getMtrId(), regDcnt.getFcdcId());
			horaClase = srvSgmt.obtenerHoraClasexHorario(hrrF);
			int tiempo = hrrF.getHracHoraClaseAula().getHoclalHoraClase().getHoclHoraFin()
					- hrrF.getHracHoraClaseAula().getHoclalHoraClase().getHoclHoraInicio();
			List<ContenidoCurricular> lstAux = new ArrayList<>();
			if (lstSgmt.isEmpty()) {
				...
				for (String selectcnt : selecCnts) {
					for (ContenidoCurricular con : lstCnt) {
						if (selectcnt.equals(con.getCncrDescripcion())) {
							...
							if (flagIni) {
								seg.setMallaCurricularParalelo(hrrI.getHracMallaCurricularParalelo());
							} else {
								seg.setMallaCurricularParalelo(hrrF.getHracMallaCurricularParalelo());
							}
	...
	public void actualizarRegistros(Asistencia regAss) {
		for (Asistencia as : lstAss) {
			if (regAss.getHorarioAcademico().getHracMallaCurricularParalelo().getMlcrprId()
					.equals(as.getHorarioAcademico().getHracMallaCurricularParalelo().getMlcrprId())) {
