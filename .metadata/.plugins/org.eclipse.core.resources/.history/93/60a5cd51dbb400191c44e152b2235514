...
import ec.edu.uce.academico.ejb.servicios.biometrico.interfaces.SrvAdministrarParametroLocal;
...
public class SrvAdministrarParametro implements SrvAdministrarParametroLocal {
	...
	// try {
	// Query query = em.createQuery("select p from Parametro as p order by
	// p.prmId desc");
	// query.setMaxResults(1);
	// parametro = (Parametro) query.getSingleResult();
	// } catch (Exception e) {
	// System.out.println("Error al consultar la secuencia del parametro: " +
	// e);
	// }
	// return parametro;
	// }
